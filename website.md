...
Pending:
- Pass 2 (separate session): `RandomizeNode.jsx` / `CounterbalanceNode.jsx` builder UI, `ExperimentBuilder.jsx` wiring (toolbar, edit panel, per-arm edge handles), balance audit view (`/admin/studies/:id/balance` or a `StudyDetailPage` tab — reuses the existing `assignment_balance` view for marginal counts, client-side pivot of raw `participant_assignments` for the stratified within-group cross-tab)
- `git push` still pending for this Pass — code is written and verified live against the deployed functions, but not yet committed to the repo
...
- [x] Experiment Builder Phase 2 Pass 1: `draw_assignment` extension, `experimentGraph.js` fork support, `materializeSchedule.ts` + `check_schedule` advance pass — implemented and verified live 2026-07-08, see §28. Not yet pushed.
- [ ] Experiment Builder Phase 2 Pass 2: React Flow fork UI (`RandomizeNode`/`CounterbalanceNode`), balance audit view — separate future session, see §28
