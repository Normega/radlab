...
- **Off-screen pause (2026-07-21)**: tab-hide (`visibilitychange`) or window `blur` during any timed phase clears every pending timer and enters `paused` — background tabs otherwise keep resolving trials as misses/correct rejections via throttled timeouts. The in-flight trial is never recorded (only `recordResponse` writes results), so "Resume watching" re-runs it after a fresh 3-2-1 countdown; the resume path always continues at the first unrecorded trial. Pause count is reported as `pauses` in the completion payload (not persisted to `pond_watch_results` — would need a column migration if wanted).
