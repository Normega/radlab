...
**Known minor UX gap, not fixed (flagged, not blocking)**: newly-added nodes (via toolbar or "+ Add arm"/"+ Add block") can render outside the current viewport, since `fitView` only runs once at canvas mount and isn't re-triggered after mutations. Recovered easily via the canvas's own fit-view control button; a real polish item for a future pass, not a correctness bug.

Not yet pushed as of this write-up.
...
- [x] Experiment Builder Phase 2 Pass 2: React Flow fork UI (`RandomizeNode`/`CounterbalanceNode`), balance audit view — implemented and verified live 2026-07-08, see §28. Not yet pushed.
