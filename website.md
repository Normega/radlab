...
Still outstanding from this thread: **there is no way to edit an accepted page.** Only `review_proposal` (which needs a *pending* version), `unpublish_page`, and the RLS helpers exist, and `wiki_pages` has no authenticated write policies — so no typo can be corrected without service-role SQL. An `edit_page(page_id, content, reason)` RPC plus a staff-only edit affordance is the missing primitive, and it's what the two duplicated pages need before they can be cleaned up. They no longer *misreport* their gaps, but they still read as three copies.
