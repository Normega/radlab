...
Built 2026-07-24 on branch `feature/academic-partition`; needs the four env vars set in Vercel, then deploy and run the plan's §5 two-paper test (same paper both modes + a second layout-heavy paper; record quality, token counts, cost per mode in `ingest-budget-estimate.md`; pick the course default mode). Phase 2 (student submission flow, review queue, wiki publishing) designed but not built.
...
- [ ] Env vars in Vercel (`ANTHROPIC_API_KEY`, `COURSE_SUPABASE_*`), merge, deploy, staff registration click-test
- [ ] Two-paper API test (same paper both modes + second layout-heavy paper); record numbers in `ingest-budget-estimate.md`; pick default mode
