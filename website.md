...
The avatar is now a `<button aria-haspopup="menu">` opening a dropdown: **My Ripple** (`/ripple`) · **Profile** (`/profile`) · **Settings** (`/settings`) · divider · **Sign out**. Closes on outside `mousedown`, on Escape, and on route change (the items are `<Link>`s, which change the route without unmounting `Nav`). Rendered in both the desktop and mobile containers — as the avatar always was — so one copy is always `display:none`; that copy is out of the tab order and the accessibility tree, so the duplication is inert.
