...
**Early-gate pull-forward** (2026-07-29): the 3-day assessment windows are catch windows, not
waiting periods. Completing the midpoint on day 14 starts Phase 2 on day 15 rather than burning the
unused window — a 29-day study instead of 31, the tail shifted by the same 0–2 days (Phase 2 day 1
is always the day after the midpoint; the final window follows Phase 2 as before). The shift lives
in `materializeSchedule`'s `dayShift`: `day_offset` is the graph's nominal calendar, and an
assessment that gates a `randomize` fork pulls every timepoint after it forward by the days its
window went unused. Rows already materialized are never moved — each timepoint re-derives the shift
from the `scheduled_date` its own first session was created with, which is what makes repeat
advance passes (and the final window, materialized a pass later) land consistently. Capped at today,
so a fork that resolves late can't back-date rows into a burst of already-due sends; a participant
who uses the full window still gets the original day-17 calendar. `complete_session_by_token`'s
design-based estimate mirrors the cap (`20260729_early_gate_advance_next_contact.sql`) so the
completion screen doesn't promise day 17 minutes before the cron pass schedules day 15. Calendar
cases are pinned in `supabase/functions/_shared/materializeSchedule.test.mjs` (`node` it directly).
...
labeling rows from `study_sessions.day_number` still show the nominal day.
...
- [~] Early-gate pull-forward (2026-07-29): an assessment window finished early now advances the study instead of waiting out the window — `materializeSchedule` `dayShift`, mirrored in `complete_session_by_token`'s estimate, 7 calendar cases pinned in `materializeSchedule.test.mjs` (see §26a). Migration applied 2026-07-29. Remaining: redeploy `check_schedule` + `auto-enroll` (the shift ships in `_shared/`, bundled per function — until then the live schedule is unchanged), then confirm on the dry-run study with a day-14 midpoint
