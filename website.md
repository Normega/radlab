...
- The Ripple card was slimmed to latest mood + streak + intention; its trend charts moved into the widget rather than drawing the same series twice on one page.
...
`/dev/insights-preview?state=rich|sparse|empty&window=month|year` mounts the widget with synthetic check-ins (dev-only, touches no database) — the states that would otherwise take a year of real use. `InsightsWidget` takes `devRows` and `initialWindow` for this, the same escape-hatch pattern as `WelcomeFlow`'s `devInitialStep`. The maths was additionally run against the real 10-row production series in a scratch script before shipping; that run is what caught the ambivalence-clamping bug and the arbitrary-tie mode label.
