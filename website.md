...
Pointer/spacebar hold drives `progressRef` 0→1 over a 3.2 s traverse (`TRAVERSE_S`); the fantasy guardian sprite frame = `round(p·41)`, exhale plays frames in reverse; the medical `CellHero` swells 0.62→1.12×. Entity travel (3.4 s spawn→resolve edge) exceeds a full traverse, so reacting at spawn always suffices. Resolution grace zone: arrivals at progress 0.4–0.6 succeed for both kinds. An internal `score` (weights +5 friend entered / +3 threat blocked / −8 breach, clamped 0–100) is **still computed and recorded** but no longer shown as a number — see Refuge state. Separate `health` (100, −10 per breach) drives only a two-tier grossness haze — kept off the score so early sessions never look punished.
