...
- Frontend-only change, not yet pushed — `npm run build` passes clean (pre-existing `numeric_es6` eval warnings and bundle-size warning only, both unrelated). Needs `git push` to deploy via Vercel; nothing here is live/checkable on the site until then.
