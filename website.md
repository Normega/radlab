...
> Added 2026-07-15. Study design + the Phase-1 reusable-materials build.
...
`online_longitudinal`, `external_enrollment_source='sona'`). Authored programmatically (like the
Liliana dry-run — a DB instance, not a migration) and structurally verified:

- **design_graph**: 383 nodes / 384 edges — `tp_baseline`(day 0) → `s_baseline` → `rnd`
  (randomize: control / self_monitoring / reflective, equal weight) → per-arm 63
  timepoint→session pairs (21 days × 3 slots at 09:00/14:00/20:00) → all three arms rejoin at
  `tp_post`(day 22) → `s_post`. **No `adherence_check` nodes** (so the shared Liliana adherence/
  withdrawal logic never fires for Zerin — see §28). Built server-side via `generate_series`
  (no 60 KB blob paste), then validated: `experimentGraph.js`-equivalent structure confirmed by
  SQL invariants (single entry `tp_baseline`, `rnd` out-degree 3, `tp_post` in-degree 3, no
  dangling edges, every session/timepoint correct out-degree) and by the generator's own
  `validate()` pass (383/384, byte-identical counts).
- **Calendar**: baseline = study day 1; dailies = days 2–22 (offset 1–21); post = day 23. The
  randomize sits after baseline, so it resolves once baseline is `completed` (the `check_schedule`
  advance pass then materializes all 63 arm dailies + post at once). Missed baseline → the
  materializer's `missed_assessment` withdrawal (correct; the only Liliana coupling is a non-fatal
  `liliana_participants` update that just logs for a Zerin participant).
- **5 session templates**: Baseline (PHQ-8 → LMS-14 → Pond Watch — **no consent step**, consent is
  now gated pre-session by `ConsentGate`/`record_consent`, and the Liliana agent's
  `20260715_remove_redundant_consent_step.sql` already stripped the consent node from Zerin
  Baseline), Post-Study (PHQ-8 → LMS-14 → Pond Watch → Debrief → Compensation), and three daily
  templates (`mood_checkin` / `mood_checkin_reflective` / `wellness_tip`). Link expiry: 72 h
  assessments, 24 h dailies.
- **study_sessions**: 191 rows, 1:1 with the graph's session nodes by `node_key` (0 gaps either
  direction — the materializer's `node_key → study_session_id` map resolves for every node).
- **Attached**: screener `langerian-mindfulness-v1` (snapshotted to `studies.screener` +
  `screener_id`), consent + debrief forms (verbatim HTML, rendered by the pre-session ConsentGate /
  Debrief step), `email_subject`/`email_body` (generic "Time for your check-in"; the unsubscribe
  footer is appended automatically by `emailTemplate.ts`, satisfying the consent form's opt-out
  requirement). Reminders on (6 h interval, max 1), `max_attempts` 2.

Reviewable live at `/admin/studies/6d3c38ce-…/design` (Experiment Builder) and `/admin/studies`.

**Still pending**: a dry-run enrollment test (enroll a throwaway SONA participant → verify baseline
materializes + link issues → complete baseline → advance pass resolves the randomize and
materializes the arm + post → cron emails). Deferred as a deliberate step; the shared scheduler it
exercises (`materializeSchedule`/`check_schedule`) is Liliana-proven and Zerin-compatible by the
analysis above.

### Open flags at build time
