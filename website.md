...
- Anthropic call: `@anthropic-ai/sdk`, model `claude-opus-4-8`, adaptive thinking, streaming (`messages.stream` + `finalMessage()`, `max_tokens` 64000), refusal/max_tokens stop reasons handled. The prompt is the **verbatim port of `psy240_schema.py`** (system prompt = compact PSY240 wiki schema; user prompt = wiki index + paper + return-only-JSON closing). **Privacy invariant: no email, name, or any identity data ever enters the Anthropic call** — only the paper and the wiki index.
