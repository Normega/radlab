...
**Conference demo (2026-07)**: `src/games/BreathBelt/BreathBeltDemo.jsx` at `/demo/breath-belt` — unauthenticated, writes nothing (no Supabase, no CSV, no-op triggers, no COM/Biopac step). Flow: pairing → real MLR calibration with review panel → 3 paced trials with post-trial SignalGraph + sync chips → 2 hardcoded change-detection trials (speed up 4s→3s, then slow down 4s→5s) with 3AFC + confidence + arousal ratings and a reveal graph → summary. `?sim=1` rehearses without a belt. Trial graphs (paced + detection) are built from the live `breathValueRef` signal sampled against the pacer (`useGraphSampler` → same `SignalGraph` as calibration), so they render both with a real belt and in sim — no dependency on raw-accel collection. Reuses `useBeltConnection`, `useTrialRunner`, `CalibrationScreen`, `SignalGraph`, shared rating scales.
