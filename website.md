...
- [~] **Onboarding Redesign v2 — web UX round (started 2026-07-30)**. First item shipped: the **revised Games page** (§12a) — catalog module, category/duration sort, derived duration+trials, public read-only guest state, two new unlock gates, daily check-in reminder. Route-level `UnlockGuard` on all three gated games and the `PlatformPage.jsx` carousel fold both landed the same day. Outstanding on it: Norm's sign-off on the four categories, the three duration buckets, and the derived per-game numbers; a check on the two About-page copy changes the fold caused ("Contact" for "First Contact", and the dropped "Required before anything else."); and a signed-in click-test — locked cards, the check-in reminder, and the bounce banners were all verified only by temporarily forcing them to render. Remaining v2 sections of the Figma file not yet started.
- [~] **Account area IA rework (2026-07-30, §12b)** — avatar dropdown replacing the Sign out button; `/ripple`, `/profile`, `/settings` split out of the old do-everything `/profile`; dead `profiles.reminder_frequency` UI deleted; change password; `delete_own_account()`. Outstanding: **execute `delete_own_account()` once on a throwaway public account** (never run), signed-in click-test of the reminder/check-in toggles and the password change, and a decision on dropping the now-unreferenced `profiles.reminder_frequency` column.
...
- [ ] Optional follow-up (§12d): retire the `radlab.vercel.app` fallbacks for `SITE_URL`/`FROM_EMAIL` in the four Edge Functions and `functions/.env.example`. Needs a redeploy of those functions.
