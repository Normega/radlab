...
- [~] `radlab.vercel.app` fallbacks for `SITE_URL`/`FROM_EMAIL` retired in code (2026-07-30) — `ripple_reminder`, `send_message`, `send-class-verification-email`, `_shared/processAdherenceWithdrawal`, plus `.env.example` and `functions/README.md`, all now default to `radlab.zone`. **Not yet redeployed**: the change is inert while the secrets are set (they are — real emails have gone out), so it takes effect at the next deploy of those four functions. `ripple_reminder`'s deployed source was confirmed byte-identical to the repo first, so a redeploy will not clobber anything.
