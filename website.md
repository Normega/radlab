...
- [ ] **Fix the Supabase URL configuration** (§12d) — Redirect URLs need `https://radlab.zone/**`, `https://www.radlab.zone/**`, `https://radlab.vercel.app/**`, `http://localhost:5173/**`; **Site URL must move from `radlab.vercel.app` to `https://radlab.zone`**, the canonical domain. Until this is done, password reset started from the real domain still lands on the root page instead of the reset form; the `index.html` rescue script catches it, but the allowlist is the actual fix. **Dashboard-only — cannot be done from code or MCP.**
