...
gains an Email column. **Deploy checklist (pending)**: apply
`20260716_participant_contact_email.sql`, redeploy `send_message` + `check_schedule` + `auto-enroll`
(the latter two bundle `processAdherenceWithdrawal`), then live-test: fresh external enrollment →
gate appears after consent → address lands on the enrollment row → a real send reaches it. Note:
existing `ext-sona-dryrun-*` participants have no contact_email, so their sends will now log
`failed` (truthful) until given one or removed.

**Remaining before pretest**: apply + live-verify the contact-email migration/redeploys (above),
...
20260716_participant_contact_email.sql    — study_enrollments.contact_email, record_contact_email RPC, gate inputs in get_session_by_token (NOT yet applied)
```
