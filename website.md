...
estimate mirrors the early half for fork gates only (`20260729_early_gate_advance_next_contact.sql`,
applied) so the completion screen doesn't promise day 17 minutes before the cron pass schedules day
15. Eight calendar cases are pinned in `supabase/functions/_shared/materializeSchedule.test.mjs`
...
- [~] Gate-relative scheduling (2026-07-29): a segment behind a gating assessment starts the day after it was completed, early or late, with the rest of the study moving by the same offset — `materializeSchedule` `dayShift`, mirrored in `complete_session_by_token`'s estimate, 8 calendar cases pinned in `materializeSchedule.test.mjs` (see §26a). Migration applied 2026-07-29. Remaining: redeploy `check_schedule` + `auto-enroll` (the shift ships in `_shared/`, bundled per function — until then the live schedule is unchanged), then confirm on the dry-run study with a day-14 midpoint
