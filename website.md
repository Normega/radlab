...
| 14–16 | **Midpoint window** — owl lead-in ("Begin Check-in", `assessment_leadin/midpoint`) → state battery + preference ranking + the feedback/choice step — hard deadline end of day 16; non-completers are withdrawn from Phase 2 (Phase 1 data retained); the fork's completion gating enforces this naturally |
...
(5–9 pass; `<5` fail_low; `>9` fail_high). Phase-1 eligibility: U of T student · **PSYC100**
...
### Phase 2 — pending (deferred per "reusable materials first")

Programmatic ~190-node `design_graph` (baseline → randomize(control/self_monitoring/reflective)
→ 63 per-arm daily timepoints → rejoin → post), create the study, attach screener/consent/
debrief, contact settings (generic reminder email **with the opt-out link** — required by the
consent form), then a dry-run enrollment test through real participant links.
