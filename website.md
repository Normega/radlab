...
- [ ] **WP4 — content sprint** (▶ in progress, started 2026-07-31). Run order, module→chapter map and the sixteen ready-to-paste citations in `docs/markdowns/psy240_wp4_runplan.md`. **Done: Modules 01, 02, 03** (plus 04 earlier) — the four foundations modules' reference runs. **Next: Module 15** (`law-and-ethics`, single target, extracted), then the disorder chapters in paper mode and native, lecture order, Module 16 last. Scope 78 generated pages / ~17 review hours; live at 66 pages with bodies, 0 published, 177 links, 0 red. Two process rules learned the hard way and now in run plan §0: reference targets against one module run **sequentially with triage between**, and **run 1's supporting pages get vetted against run 2's catalogue target** before acceptance. Extracted mode confirmed 4.2× cheaper than native on prose modules; native stays for the disorder chapters. `integrative-model` moved to the uncovered-topics list — the textbook has no integrative framework to give it.
