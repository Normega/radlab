...
- The Ripple card was slimmed to latest mood + streak + intention; its trend charts moved into the widget rather than drawing the same series twice on one page. **The Ripple portrait sits to the left of that information** (2026-07-31, per the `CheckinReminder` frame), replacing the small mood-dot grid that used to hold that slot — the grid was a third copy of a chart the widget now owns properly. The face is driven through `composite_label → LABEL_TO_ID → EMOTIONS`, the same lookup `RevealStep` uses, so a mood renders the same expression on the dashboard as it did the moment it was recorded; feeding `composite_x/y` straight into `valence`/`arousal` would have been a second, quietly different mapping for the same data. Intensity falls back to composite magnitude because `zone` is not stored on the row.
