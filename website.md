...
| `/welcome` | `WelcomeFlow` (`src/ripple/`) | Auth only — Ripple WP1 public-tier onboarding: consent + ToS → demographics → bridge to avatar creation. New public users (no avatar, `onboarding_complete=false`) are redirected here by `ProtectedRoute`; existing users bypass until the WP2 migration beat. Own `ErrorBoundary label="Ripple"` partition (spec: `docs/markdowns/ripple_spec.md`) |
...
- **v1 (primary)**: quality = (z(delta_stress) + z(appraisal)) / 2, z-scored within-person over
  completed Phase 1 sessions; practices ranked by mean quality (deterministic tie-breaks:
  v1 → delta → helpful → seeded hash; `low_n` flag below 2 usable sessions)
- **v2 (pilot comparison)**: rank by mean delta_stress alone — stored alongside v1 in every
  snapshot for the August bake-off (WP-L6: rank agreement + leave-one-out stability decide which
  ships; freeze `metric_version` before September recruitment)
...
- [~] WP1 (code complete 2026-07-12, **migration not yet applied, not verified live**) — `20260712_ripple_wp1.sql` (`ripples`, `ripple_checkins`, `consents` + RLS; see manifest), `/welcome` flow (`src/ripple/WelcomeFlow.jsx`: intro → consent + minimal ToS (versioned draft copy in `src/ripple/consentDocs.js`, pending ethics review) → demographics (reuses `demographics` table + instrument) → bridge to `/profile/avatar`), `ProtectedRoute` routes **new** public users (no avatar, `onboarding_complete=false`) to `/welcome`; existing users bypass until WP2's migration beat. Own lazy chunk + `ErrorBoundary label="Ripple"`. Remaining before ticking: apply migration, click-test the flow live end-to-end
- [ ] WP2 — Ripple identity: ExpressiveAvatar unification, appearance + naming UI, header swap, existing-user migration beat
- [ ] WP3 — check-in core (condensed Still Water flow, points, write-time streaks, `/checkin`)
- [ ] WP4 — login greeting, prompt cadence, `/ripple` settings + disable path, rotating item engine
- [ ] WP5 — dashboard personal trends (folds into P2 dashboard wiring)
- [ ] WP6 — email reminders + unsubscribe (the "opt-in contact mechanism" item); leaderboard/streak surfacing decision
...
- [ ] Phase 2 remaining: Claude summarization Edge Function, avatar wall presence
