...
|---|---|---|
| `Farb_CV_2026.pdf` | `pi.links` → "CV (PDF)" | **Redacted copy.** Home address and home phone are removed from page 1; the source on Norm's drive is unredacted. When refreshing the CV, re-run the redaction — do not drop the raw export in. |
...
**Mode is per module, and the rule is slug canonicality, not source type.** The foundations modules run in *reference* mode because their slugs are our invention and paper mode will never land on them — proven here, where Module 01's paper run produced six good pages and zero foundations. The disorder chapters stay *paper* mode: disorder names are canonical so the slugs converge on their own, one paper run yields ~18 pages where reference mode needs a run per target (Module 13 alone would be 11), and reference mode deliberately suppresses the supporting concept/treatment/debate pages that make the wiki a wiki.
