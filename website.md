...
3. Supabase emails a recovery link (template + redirect URL allowlist configured in the Supabase dashboard → Auth → URL Configuration). The email uses the **branded template** in `supabase/auth_email_templates/reset_password.html` (2026-07-10) — logo + wordmark header, same card/CTA design as the study emails in `_shared/emailTemplate.ts`, subject "Reset your RADlab password" — replacing the stock two-line Supabase template that was generic enough to junk-folder. Template body is pasted into the dashboard (Auth → Emails → Reset Password); the folder's README also documents pointing Supabase Auth at the existing Resend account via custom SMTP (Project Settings → Authentication → SMTP), which is the actual deliverability fix — default auth mail comes from Supabase's shared `noreply@mail.app.supabase.io` pool with no domain-aligned SPF/DKIM. Dashboard/SMTP application pending — see the README checklist.
