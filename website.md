...
- **Off-by-one in the daily day numbering (found in the dry run — applies to the live study).**
  Baseline is `study_day` 1, so the first daily timepoint (`day_offset` 1) is `study_day` 2 and
  the dailies run 2–22. The daily widgets key off `study_day`: `WellnessTipStep` looks up
  `CONTROL_TIPS[study_day]` (keyed 1–21) and both widgets label the screen "Day N". Result: the
  first control daily shows "Day 2" with day-2's tip (day-1's "Drink water" tip is never shown),
  and the last daily (`study_day` 22) has no matching tip → generic fallback, so the Day-21
  "final / complete the post-study survey tomorrow" script never appears. Fix is a design choice
  (renumber dailies to start at day_offset 0, or subtract 1 when indexing tips/label) — deferred
  to Zerin/Norm.
- Minor: daily reminder greeting is "Hi SONA," (uses the SONA source label, not a real name) for
  anonymous external participants; `participant_compensation.participant_id` is left null (the row
  is keyed by `enrollment_id`).
...
- **`SessionEntry.jsx` v8 copy**: next row today → "Your next session is later today — watch for an email around 6:00 PM"; tomorrow → "tomorrow (Thursday, July 16)"; later → "in N days ({date})", all noting a link will be emailed. No row but `has_more` → "You'll receive an email when your next session is ready" (fork gate — check_schedule's advance pass materializes the winning arm within 15 min, and the email goes out on its scheduled day). `has_more === false` → "you have completed the final session of this study!". `null`/missing → old generic text. The SONA `completion_redirect_url` path is untouched. Dates parsed as local time, not `new Date('YYYY-MM-DD')` (UTC-midnight off-by-one west of Greenwich).
