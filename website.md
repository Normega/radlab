...
- [ ] WP-L2: check-in packages into all training session templates + align `/admin/training` wrapper demo with the canonical packages
- [ ] WP-L3: `liliana_session_metrics` view, `liliana_midpoint_feedback` snapshot table, summary/decision RPCs, `draw_assignment` kind-filter patch
- [ ] WP-L4: `MidpointStep` component (feedback / control-display / owl-placement arms)
- [ ] WP-L5: study wiring (`midpoint_group` slot, practice fork node) + three-arm end-to-end dry run + data export coverage
