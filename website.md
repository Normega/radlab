...
- [ ] **WP4 — content sprint** (▶ in progress, started 2026-07-31). Run order, module→chapter map and the sixteen ready-to-paste citations in `docs/markdowns/psy240_wp4_runplan.md`; module PDFs in `F:\gits\radlab_project\PSY240resources\`. **Done: the whole foundations block — Modules 01, 02, 03, 15** (plus 04 earlier), and **Module 07 (anxiety), the pilot disorder chapter** — 14 pages, 5 Tier A, all triaged, and the first run to exercise the `action: update` merge path. **Next: run 6 — Module 09 (OCD), paper, native**, then 08, 05, 06, 10, 11, 12, 14, 13, Module 16 last. Scope 78 generated pages / ~17 review hours; live at 84 pages with bodies, 0 published, 252 links, 0 red. Two process rules learned the hard way and now in run plan §0: reference targets against one module run **sequentially with triage between**, and **run 1's supporting pages get vetted against run 2's catalogue target** before acceptance. Extracted mode confirmed 4.2× cheaper than native on prose modules; native stays for the disorder chapters. `integrative-model` moved to the uncovered-topics list — the textbook has no integrative framework to give it. **The 16 topic overviews moved to the reference pass** — two paper-mode chapter runs produced zero of them, confirming that catalogue-invented slugs need a named target.
