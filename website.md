...
**Status**: Designed 2026-07. **Phase 1 WP1–WP3a implemented and verified live 2026-07-11** (schema+RLS, join/verify, per-class console, lab-wide class/instructor admin at `/lecture-lounge/admin`) — see the header update at the top of this document for what shipped and what changed from the design below during implementation (verification is account-level via `profiles`, not per-class via `class_members` as the schema table further down still states; `class_admins.user_id` not `profile_id`; `classes` has no `join_code`/`active` columns). WP3b (mobile remote), WP3c (projector screen), WP4 (broadcast state), WP5 (check-in flow/results) not yet built. The schema tables and decisions below have not been fully reconciled against the live implementation — treat this section's *intent* as current but verify exact column names/types against `supabase/migrations/2026071*_lecture_lounge_*.sql` before relying on specifics. First platform use of the Anthropic API (still pending — Claude summarization is WP4/Phase 2 scope, not yet built).
...
- [ ] WP4 broadcast state machine + student check-in state machine
- [ ] WP5 check-in flow (mood/pacing/prompt/question) + results view
