...
container as the "Part N of M · Item X of Y" row — so the instruction/scale reminder scrolls with
nothing, staying pinned alongside the counter. Collapsed by default to one line: `questionnaire.instructions`
(truncated with an ellipsis if long) plus the current item's scale endpoints, e.g. "…using the scale
below. · Not at all → Extremely". Tap anywhere on the frame to expand it — shows the full instruction
text plus every scale label (not just the endpoints), reusing the same value/label row layout as
`InstructionScreen`'s and `ScaleChangeScreen`'s scale cards.

On a `scale_change` slide, the *next* item slide's `InstructionFrame` receives that segment's labels
(the same `effectiveLabels()` result already computed for `LikertItem`), so the reminder's anchors
always match the scale actually in effect — this is the main motivation: `ScaleChangeScreen` itself is
shown only once, and a participant several auto-advancing items past it has no other way to recall
which end of the scale means what. No definition-level opt-out — this is default rendering behavior
for every `likert`-type questionnaire, gated only on `instructions` being non-empty (renders nothing
otherwise). Not rendered on `instruction`/`scale_change`/`checklist` slides — same `itemIndex`-gated
visibility `ProgressLabel` already had.
...
missed dailies no longer permanently block Phase 2 while a missed midpoint still structurally
enforces withdrawal; **reminders** — `last_sent_at` cadence anchor, same-evening nudge for dailies,
...
distress-flag monitoring + adherence/credit tooling (methods-doc requirements), author the real
study (duplicate the proven dry-run graph), delete the dry-run study + `ext-sona-dryrun-*` accounts.
