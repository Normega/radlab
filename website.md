...
- **Platform (`/games/*`, `/platform`, `/login`, `/signup`, `/dashboard`, `/profile*`, …)**: `Nav.jsx` — **rewritten 2026-07-16 as the Onboarding Redesign v1 Header (Guest/User, Figma 161:1068)**. Consistent-header rule: Dashboard · Games · About (→`/platform`) always visible as `ButtonNav` pills; for guests **Dashboard** is **visible-but-inert** (muted, `title` hint, no navigation — DRIFT-REPORT §9 Q7). **Games became a real link for guests 2026-07-30** when `/games` went public read-only (§12a) — the Onboarding Redesign v2 games page draws a guest state, so the inert treatment now applies to Dashboard only. Guest actions: Log in (SecondaryCTA) + Join free (PrimaryCTA). User actions: Sign out + `RippleAvatar` in a 2px pink ring (→`/profile`). Admin pill for lab/super-admin users (live-site necessity, not in Figma). Mobile (<`md`): pills collapse into a hamburger drawer; Join free / avatar stay in the bar (Dev Spec §6.3 recommendation, provisional pending a real mobile design).
