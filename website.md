...
**Rejecting a proposal leaves the page behind, and one such shell is currently a link to nowhere.** The two duplicate proposals rejected on 2026-07-31 (`history-of-mental-illness`, `research-methods-in-psychopathology`) still exist as `status='proposed'` rows with `content IS NULL` — rejection marks the *version*, not the page it created. Harmless for the model, which indexes only accepted content, but `what-is-abnormal` links to `history-of-mental-illness`, and because the target row exists that link resolves: it counts as blue, renders as blue, and leads to an empty page. **So "0 red links" currently certifies one link too many.** Two fixes, and both are wanted: retarget that link to `historical-traditions` (which holds the content) and archive the two shells; and make rejection of an `action: new` proposal clean up the page row it created, so the residue stops accruing one pair per foundations run.

Live state: **84 pages with bodies**, 0 published, 2 archived, 252 wikilinks and 0 red links *(with the caveat above)*, review queue clear, catalogue at 4 complete / 21 gaps / 100 unwritten, 22 ingest jobs totalling ~1.17M input and ~220k output tokens.
