-- complete_session_by_token: the "next contact" must actually be in the future.
--
-- Bug (found by Norm 2026-07-25): after finishing the 20:00 check-in, the
-- completion screen said the next session was "later today at 2:00 PM" — a slot
-- that had already come and gone.
--
-- The next-contact query selected the earliest row with status IN
-- ('pending','link_sent','unlocked') ordered by (scheduled_date, send_time),
-- with no filter on that pair being in the future. In a several-times-a-day
-- study, a same-day slot the participant missed sits at 'link_sent' forever:
-- check_schedule's step 0b only promotes rows to 'missed' when
-- scheduled_date < today, so intra-day misses keep a non-terminal status until
-- the next day. That stale earlier row then sorts ahead of every genuinely
-- upcoming one and gets reported as "next".
--
-- Confirmed live: participant 9c4c289f completed 09:00 and 20:00, missed 14:00
-- (link expired, row still 'link_sent') — and the 14:00 row was returned as the
-- next contact.
--
-- Fix: same future-time predicate already used by next_pending_session
-- (20260725_completed_link_distinct_from_expired.sql), in lab time. Rows that
-- fall away as a result correctly fall through to the design-graph estimate
-- below, which is the intended path when nothing upcoming is materialized.
--
-- Otherwise identical to the 20260720_completion_next_contact_estimate.sql
-- definition.
