-- The source comparison belongs to REVIEW, not to acceptance.
--
-- As first built, the model read the paper only when a TA accepted a
-- submission — so the one thing precheck cannot answer ("does the summary say
-- what the paper says?") arrived as a notice AFTER the decision it should have
-- informed. This stores the drafted section and the verdict on the claim so
-- the queue can show them beside the student's own words, and so accepting
-- files the draft review already produced instead of paying for a second
-- identical model call.

alter table gap_claims add column if not exists integration_draft text;

-- integration_draft joins the bookkeeping columns the guard lets through when
-- radlab.claim_flow is set (see 20260903_claim_bookkeeping).
...
declare
  ...
  v_book  text[] := array['notified_at', 'source_kind', 'source_fulltext', 'source_url_full',
                          ...
                          'integration_version_id', 'integration_draft'];
begin
...
-- Replaced, not overloaded: two candidates differing only by a defaulted
-- argument make the PostgREST call ambiguous.
drop function if exists public.record_claim_integration(uuid, text, text, uuid);
...
revoke all on function public.record_claim_integration(uuid, text, text, uuid, text) from public, anon, authenticated;
grant execute on function public.record_claim_integration(uuid, text, text, uuid, text) to service_role;

-- New columns are APPENDED: CREATE OR REPLACE VIEW cannot rename or reorder
-- existing ones.
