...
| `20260712_admin_user_management.sql` | **NOT YET APPLIED** — written 2026-07-12 (super-admin `/admin/users`: `admin_list_users`, `admin_set_user_role` (lab↔public only, no super/participant/self targets), `admin_delete_user` (transactional, no super/self targets), privesc trigger gains `is_super_admin()` exemption matching the 20260611 RLS policy). Apply via SQL editor, then replace this row with evidence |
| `20260712_ripple_wp1.sql` | **NOT YET APPLIED** — written 2026-07-12 on branch `claude/wellness-buddy-onboarding-usteph` (Ripple WP1: `ripples`, `ripple_checkins`, `consents` + RLS). Apply via SQL editor or MCP `apply_migration` before merging/deploying the `/welcome` flow, then replace this row with evidence |
