-- ============================================================
-- Fix: uploaded questionnaires in session templates
-- Apply once in the Supabase SQL Editor.
-- ============================================================
--
-- session_template_nodes can reference EITHER an activities row (activity_id)
-- OR an uploaded questionnaire (questionnaire_id). The original
-- get_session_by_token RPC inner-joined activities only, so questionnaire-only
-- nodes (e.g. PHQ4) were silently dropped from the session. This replaces the
-- function to LEFT JOIN both and synthesize an activity-shaped object
-- ({category:'questionnaire', subcategory: <slug>}) for questionnaire nodes,
-- which is exactly what StepDispatcher keys off. Everything else is unchanged.
