...
-- ── 1. Add screener column to studies ────────────────────────────────────────
ALTER TABLE studies
  ADD COLUMN IF NOT EXISTS screener jsonb;

-- ── 2. screener_results table ─────────────────────────────────────────────────
-- Only pass/fail outcome and timestamp are recorded — no item-level data.
...
CREATE TABLE IF NOT EXISTS screener_results (
  ...
  phase2_outcome  text        CHECK (phase2_outcome IN ('pass', 'fail_low', 'fail_high')),
...
-- ── 3. Update get_session_by_token to expose screener ─────────────────────────
