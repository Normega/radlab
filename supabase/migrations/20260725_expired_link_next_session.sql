-- Expired-link landing: return the participant's next scheduled session
-- alongside the 'expired' error, so SessionEntry can tell them when the next
-- link arrives instead of dead-ending them.
--
-- Before this, an expired token returned a bare {'error': 'expired'} with no
-- participant context, and SessionEntry's catch-all `data.error` branch mapped
-- it to 'not_found' — so someone who clicked a day-old link was told "This link
-- is not valid", with no indication they were still enrolled. (The client's
-- `link.status === 'expired'` branch never ran: this expiry check returns first.)
--
-- next_session is the earliest still-pending row for this participant+study
-- whose (scheduled_date, send_time) is in the future in lab time, or null when
-- there is none — study finished, withdrawn, or the next segment is behind an
-- unresolved fork gate and hasn't materialized yet. Lab time is
-- America/Toronto, matching LAB_TIMEZONE in the check_schedule function.
--
-- Otherwise identical to the 20260716_participant_contact_email.sql definition.
...
BEGIN
  ...
  IF v_link.expires_at IS NOT NULL AND v_link.expires_at < now() THEN
    ...
    -- Next upcoming session, for the "you're still enrolled, here's when the
    -- next one opens" copy. Suppressed for withdrawn/completed enrollments,
    -- which can still have stale pending rows we must not advertise.
      ...
      WHERE ps.participant_id = v_link.participant_id
        ...
        AND NOT EXISTS (
          SELECT 1 FROM study_enrollments se
           WHERE se.profile_id = v_link.participant_id
             AND se.study_id   = v_link.study_id
             AND se.status IN ('withdrawn', 'completed')
        )
      ORDER BY ps.scheduled_date, ps.send_time
      LIMIT 1;

    RETURN jsonb_build_object('error', 'expired', 'next_session', v_next);
  END IF;
