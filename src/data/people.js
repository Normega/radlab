// src/data/people.js
// Edit this file to update the People page.
// Photos live in public/images/people/ — download from radlab.zone/images/people/ and place there.

export const pi = {
  name: "Norman Farb",
  credentials: "PhD",
  role: "Principal Investigator",
  photo: "/images/people/norm2.jpg",
  bio: "Norm is an Associate Professor of Psychology at the University of Toronto Mississauga, where he directs the Regulatory and Affective Dynamics laboratory. He studies the psychology of wellbeing, focusing on mental habits, such as how we think about ourselves and interpret our emotions. He is particularly interested in why people differ in their resilience to stress, depression, and anxiety. His work currently explores online training to support wellbeing, as well as neuroimaging to understand how emotional reactions predict mental health over the lifespan. Hobbies include yoga, frisbee, sci-fi novels, and reality TV shows about cooking.",
  links: [
    { label: "CV (PDF)", url: "/docs/Farb_CV_2026.pdf" },
    { label: "Google Scholar", url: "https://scholar.google.com/citations?user=N3eZJd4AAAAJ" },
    { label: "LinkedIn", url: "https://ca.linkedin.com/in/norman-farb-4a412516" },
  ],
};

export const gradStudents = [
  {
    name: "Thomas Anderson",
    role: "PhD Candidate",
    photo: "/images/people/thomas.jpg",
    bio: "Thomas completed his BA in psychology with computer science minor at the University of Waterloo and his MA in psychology at the University of Toronto. He is currently a PhD Candidate at the University of Toronto and is the Research Director and Co-founder of the Psychedelic Studies Research Program (PSRP). While completing his PhD in cognitive neuroscience, he co-founded the PSRP after publishing one of the first studies on psychedelic microdosing. Thomas has published scholarly articles on attention, meditation, psychedelics, and neuroimaging. His PhD focuses on understanding meta-awareness: the experience of consciously noticing that our behaviours are not aligned with our intentions, goals, and values. He has a strong commitment to open science. Thomas enjoys dark chocolate, classic literature, and, in general, decadence.",
    links: [
      { label: "Website", url: "https://thomasanderson.ca/" },
      { label: "OSF", url: "https://osf.io/98meg/" },
    ],
  },
  {
    name: "John Eusebio",
    role: "PhD Candidate",
    photo: "/images/people/john.jpg",
    bio: "John received his BA in Psychology with a specialization in Developmental and Cognitive Neuroscience from Western University and his MA in Psychology from the University of Toronto. He is currently completing his PhD in Psychology. His main areas of focus are in the field of sustained attention and mind wandering — particularly the neurocognitive mechanisms behind what causes our thoughts to drift away from the task at hand. John also has experience in meditation research and how it affects sustained attention. His hobbies include strength training, programming, playing with his dog Kobe, overly-detailed movie analyses, and watching cooking videos.",
    links: [],
  },
  {
    name: "Sandy Luu",
    role: "PhD Candidate",
    photo: "/images/people/sandy.jpg",
    bio: "Sandy received her HBSc in Life Sciences with a specialization in sensory motor systems from McMaster University and her MSc in Neuroscience from Queen's University. She is currently a PhD candidate at the University of Toronto, where her research focuses on examining linkages between perfectionism, rumination, and self-regulation behaviours. Sandy's work also aims to explore interactive mindfulness-based interventions, drawing on psychotherapeutic models such as CBT to support student mental health. In her spare time, you can find her doing yoga and pilates, watching reality TV, Quora-surfing, and making playlists.",
    links: [
      { label: "LinkedIn", url: "https://www.linkedin.com/in/sandyluu11" },
    ],
  },
  {
    name: "Liliana Wu",
    role: "PhD Student",
    photo: "/images/people/liliana.jpg",
    bio: "Liliana received her B.Sc. in Psychology from the University of Toronto. She is currently a PhD student in Psychology at the University of Toronto, where her research focuses on understanding the structural and functional neural mechanisms underlying depression vulnerability. Her work aims to develop personalized, web-based micro interventions to enhance well-being among university students. Outside of her academic pursuits, Liliana is passionate about fitness, discovering healthy recipes, and binge-playing PS5 games.",
    links: [
      { label: "LinkedIn", url: "https://www.linkedin.com/in/lilianacwu/" },
      { label: "Google Scholar", url: "https://scholar.google.ca/citations?user=8L0pVSgAAAAJ&hl=en" },
    ],
  },
  {
    name: "Zoey Zuo",
    role: "MA/PhD Student",
    photo: "/images/people/zoey.jpg",
    bio: "Zoey received her B.S. in Cognitive Science from the University of California, Los Angeles and worked as a research coordinator at Johns Hopkins University. She is currently an MA/PhD student in the Psychological Clinical Science program at the University of Toronto Scarborough. Her research interests include using machine learning tools to characterize brain activity during different states of attention and understanding how contemplative practices promote well-being. Her hobbies include growing plants and fungi, consuming narratives in any form, and looking at ceramics, paintings, and illustrations.",
    links: [],
  },
  {
    name: "Gerold Song",
    role: "UX/UI Designer",
    photo: "/images/people/gerold.jpg",
    bio: "Gerold completed his BA in Psychology at the University of Waterloo and is currently pursuing a Master of Information at the University of Toronto focusing on UX Design and Data Science. His interests in psychology and mindfulness informs his desire to design accessible, human-centric health and wellbeing solutions, currently taking shape through a platform design project for RADlab. His hobbies include staying active, speciality coffee, and cooking for his friends.",
    links: [],
  },
  {
    name: "Jaafar Al-Tabakchali",
    role: "Undergraduate Student",
    photo: "/images/people/jaafar.jpg",
    bio: "Jaafar is an undergraduate student at the University of Toronto Mississauga interested in the relationship between physiological states and psychological well-being. His broader interests are how health behaviours such as exercise, nutrition, and contemplative practices influence cognition and mental health. He is particularly interested in combining research, technology, and public communication to make evidence-based health information more accessible. Outside of research, he enjoys going to the gym, being in nature, and cooking steaks.",
    links: [],
  },
];

export const alumni = [
  {
    name: "Geissy Araújo",
    role: "PhD Student (Alumni)",
    photo: "/images/people/geissy.png",
    bio: "Geissy received her MSc in Neuroscience from the Federal University of Rio Grande do Norte, Brazil, and pursued her PhD in Neuroscience. Her research focused on sustained attention, interoception, and stress response after a brief mindfulness-based intervention.",
    links: [],
  },
  {
    name: "Philip Desormeau",
    role: "PhD (Alumni)",
    photo: "/images/people/phil.jpg",
    bio: "Phil received his BA from Concordia University and completed his PhD in the Psychological Clinical Science program at the University of Toronto Scarborough. His research interests revolve around cognitive-affective and neurobiological mechanisms underlying problem-focused coping and affect regulation.",
    links: [],
  },
  {
    name: "Le-Anh Dinh-Williams",
    role: "PhD (Alumni)",
    photo: "/images/people/leanh.jpg",
    bio: "Lê-Anh received her BSc from McGill University and MSc from the University of Montreal. She completed her PhD in clinical psychology at the University of Toronto and a postdoctoral fellowship at McGill University. She is currently working as a clinical research fellow at Imperial College London and as a clinical psychologist at Connecte. Her research focuses on mindfulness-based and psychedelic therapeutic approaches for depression.",
    links: [],
  },
  {
    name: "Jordan Livingston",
    role: "Affiliated Researcher",
    photo: "/images/people/jordan.png",
    bio: "Jordan received her PhD with Prof. Elliot Berkman at the University of Oregon. Her research focuses on the social neuroscience of the self and personal identity.",
    links: [],
  },
  {
    name: "Kyle Logie-Hagen",
    role: "PhD Student (Alumni)",
    photo: "/images/people/kyle.jpg",
    bio: "Kyle completed his MSc in Neuroscience at McGill University and began a PhD in Psychology at the University of Toronto. His interests span interoception, contemplative practices, and mental and physical health. He completed his undergraduate studies at the University of Western Ontario.",
    links: [],
  },
  {
    name: "Katie Walsh",
    role: "MA/PhD (Alumni)",
    photo: "/images/people/katie.jpg",
    bio: "Katie received her BA with honours at York University and her MA/PhD from the University of Toronto. Her research interests include the biological basis of depression and anxiety in relation to functional connectivity of brain networks, and how mindfulness-based cognitive therapy produces positive change.",
    links: [],
  },
  {
    name: "Yiyi Wang",
    role: "PhD (Alumni)",
    photo: "/images/people/yiyi.jpg",
    bio: "Yiyi received her BA from the University of California, Irvine and her MS from the University of Southern California. She completed her PhD in Psychology at the University of Toronto in 2024, focusing on mindfulness and wellbeing. Her research explored technology-based mindfulness practices, emotion regulation, and online mental health interventions including chatbots. Yiyi now works as a data analyst in industry.",
    links: [
      { label: "Website", url: "https://yiyi-wang.web.app/" },
    ],
  },
];
