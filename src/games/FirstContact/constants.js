export const SYNC_THRESHOLD            = 0.80;
export const BUFFER_SIZE               = 4;
export const MIN_CYCLES_BEFORE_COMPLETE = 4;
export const BREATH_DURATION_MS        = 4000;
export const SCALE_AMPLITUDE           = 0.15;
export const AURA_MAX_OPACITY_GAME     = 0.60;
export const AURA_MAX_OPACITY_EBB      = 0.35;

export const COPY = {
  intro_first:           'Your Ripple is waiting. Begin breathing to make contact.',
  intro_returning:       'Welcome back. Breathe to deepen your connection.',
  complete_first:        'Initial contact established. Your Ripple is with you.',
  complete_returning:    'Connection deepened.',
  btn_continue_onboarding: 'Continue to Ebb & Flow',
  btn_done_standalone:   'Done',
  games_tagline_first:   'Begin here. Meet your Ripple for the first time.',
  games_tagline_deeper:  'Return to strengthen your connection.',
};
