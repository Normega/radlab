...
// Student error reports, as a panel rather than a page (2026-09-09).
//
// This was /academic/:code/reports, its own route and its own button on Course
// Home. It is the same gesture as a submission — a student sent a TA something
// and the TA judges it and answers — so a TA had two inboxes to remember, one
// of which had never held a row. Folded into the Student queue as a tab; the
// old route redirects, and the triage logic below is unchanged.
//
// Three exits per report, and each one closes the loop the student can see:
...
//              flags the gap seeded from the report, links it, resolves. The
//              reporting student claims it on the board; their submission
...
export default function ReportsPanel({ courseClient, courseId, wikiBase, onCountChange }) {
  ...
  useEffect(() => {
    ...
    return () => { live = false }
  }, [courseClient, courseId, reload, onCountChange])
