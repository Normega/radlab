...
| **One course never wears another's identity** | `scripts/course-leak-audit.mjs` (prebuild, ratcheted) | The URL names the course; per-course config lives in registries (`courseFeatures.js`, reply-to/DECKS maps, `weekIcons`); shared code never hard-codes a course code, and NEVER as a fallback when no course resolves — be generic (`/academic/fieldguide/*`, plain "Field Guide" branding) or fail. Every 2026-09-05 cross-course bug (PSY240-branded PSY309 sign-in emails, `?? 'psy240'` retry links, alphabetical enrollment tiebreaks) broke this rule. New course mentions in src/ or api/ fail the build; `--update` re-baselines after a deliberate registry change. |
| Feature switches | `src/academic/courseFeatures.js` | only to turn something OFF (default is full-featured). `contributions: false` = participation-only tracking page; `gaps: false` = no gap board, no per-page gap boxes or flag control; `ingest: false` = no ingest portal or links, edit hint drops the ingest pitch. All three off for PSY309 (its guide is authored whole; direct /gaps and /ingest URLs bounce to the wiki index). `quizGraded: false` = weekly quizzes are ungraded practice: runner, lobby card and archive drop every credit / grace-week / 75% line and read "practice · open until <close>" (PSY309) |
