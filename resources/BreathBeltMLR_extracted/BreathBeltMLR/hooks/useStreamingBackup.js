...
// ── useStreamingBackup ────────────────────────────────────────────────────
//
// Streams CSV rows to local files via File System Access API, running in
// parallel with Supabase writes. Provides a live local backup in case of
// connectivity issues.
//
// Usage:
//   const backup = useStreamingBackup()
//   await backup.initBackup(participantId)      // opens directory picker once
//   await backup.flushAccel(pendingAccelRows)   // called after each trial
//   await backup.flushHR(pendingHRRows)
//   await backup.appendTrial(trialRow)
//   await backup.appendQuest(questRow)
//
// File naming: {participantId}_{ISO-timestamp}_{type}.csv
// Files are opened once and appended to throughout the session.
// If the API is not available (non-Chrome), initBackup() returns false gracefully.
...
export function useStreamingBackup() {
  ...
  // ── initBackup ─────────────────────────────────────────────────────────
  // Call once at session start. Opens directory picker, creates 4 CSV files.
  // Returns true on success, false if API unavailable or user cancels.
  ...
  const initBackup = useCallback(async (participantId) => {
    ...
    try {
      ...
      await _append(trials, 'phase,trial,condition,base_period_s,change_period_s,start_ms,end_ms,peak_error_ms,bt_baseline_period_ms,bt_condition_period_ms\n')
      ...
      // User cancelled picker or permission denied — not an error, just degrade gracefully
  ...
  // ── flushAccel ─────────────────────────────────────────────────────────
  // Flush an array of accel rows to file. Called after each trial.
  ...
  // ── appendTrial ────────────────────────────────────────────────────────
  // One row per trial. btBaselinePeriodMs / btConditionPeriodMs may be null.
  ...
  // ── appendQuest ────────────────────────────────────────────────────────
  // One row per QUEST trial. delta is in absolute seconds (platform convention).
  ...
  const appendQuest = useCallback(async (row) => {
    ...
    const line = [
      row.trial_number ?? '',
      row.condition ?? '',
      row.delta_s != null ? row.delta_s.toFixed(4) : '',   // absolute seconds
      row.response ?? '',
      row.correct ? 1 : 0,
      row.confidence ?? '',
      row.arousal ?? '',
...
// Uses File System Access API writable stream, seeking to current file end.
