    ...
    return (
      <Layout title="Phase 3 — Detection thresholds">
        <StaircaseScreen
          ...
          onComplete={async (trials, questState) => {
            convergenceRef.current = useBeltQuestStaircasesConvergence(questState);
...
// Helper used to extract convergence for the summary screen
// (avoids re-instantiating QUEST just to read the final values)
function useBeltQuestStaircasesConvergence(questState) {
  if (!questState) return null;
  // Reconstruct minimal convergence from saved state
  // (proper computation done in useBeltQuestStaircases during phase 3)
  return {
    faster: { meanDeltaSec: null, sd: null },
    slower: { meanDeltaSec: null, sd: null },
  };
}
